...
function ListReorder({ nextpage }) {
  ...
  const [answers, setAnswers] = useState(() =>
    loadLocalStorage(answersLocalStorageKey)
  ); // Houd de antwoorden bij
  ...
  const handleAnswerSelection = (questionIndex, order) => {
    ...
    updatedAnswers[questionIndex] = order.map((answer) =>
                        ...
                        <Draggable key={item} draggableId={item} index={index}>
                          {(provided, snapshot) => (
                            <div
                              ...
                              className={`draggable drag-title ${
                                snapshot.isDragging ? "isDragging" : ""
                              }`}
                            >
                              {/* Voeg het icoon toe aan de linkerkant */}
                              <img
                                src={dragIcon}
                                alt="drag icon"
                                className="drag-icon"
