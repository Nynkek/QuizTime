...
function MatchValuesApp({nextpage}) {
    const [states, setStates] = useState(() => {
        ...
        return savedStates ? JSON.parse(savedStates) : questions_drag_and_drop.map((q, questionIndex) => {
            const state = { [`droppable-${questionIndex}-1`]: q.question_options };
    ...
    useEffect(() => {
        const allQuestionsFilled = states.every((state, index) =>
            Object.keys(state)
                .filter(key => key !== `droppable-${index}-1`).length ===
            questions_drag_and_drop[index].answer_options.length
        );
    ...
    const checkAnswers = () => {
        ...
        questions_drag_and_drop.forEach((question, index) => {
            ...
            if (userAnswers.length !== question.answer_ordered.length) {
                ...
                console.log(`Fout: Niet alle antwoorden ingevuld voor vraag ${index + 1}`);
                return;
            }
            ...
            {questions_drag_and_drop.map((question, questionIndex) => {
                ...
                const onDragEnd = (result) => {
                    ...
                    if (source.droppableId === destination.droppableId) {
                        ...
                        setStates(states.map((s, idx) => (idx === questionIndex ? newState : s)));
                    } else {
                        const result = move(
                            currentState[source.droppableId],
