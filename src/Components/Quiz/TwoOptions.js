...
function TwoOptions({ nextpage }) {
  ...
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(() => {
    ...
    return (
      parseInt(localStorage.getItem("quizCurrentQuestionIndex_2o"), 10) || 0
    );
  });
  ...
  const handleAnswerSelection = (questionIndex, selectedAnswerIndex) => {
    ...
    if (
      questionIndex < questions_two_options.length - 1 &&
  ...
  const checkAnswers = () => {
    ...
    // Als er geen fouten zijn, +10 per vraag optellen
    if (!hasErrors) {
      newScore += questions_two_options.length * 10; // +10 voor elke vraag
      navigate(nextpage); // Ga door naar de volgende pagina
    }
